...
// 修改密码
export const onSubmitPaword = (data,callback) => {
    let old_password = getAESEncrypt(data.password),
        new_password = getAESEncrypt(data.password2)
    let send = new Packet()
        send.AddDataset()
        send.AddItem(0, 'old_password', old_password)
        send.AddItem(0, 'new_password', new_password)
    let fetchOptions = getFetchOptions('POST', send)
    fetch(api.modifyPassword, fetchOptions).then(response => {
        if (response.status !== 200) {
            message.error(response.status)
        } else {
            response.json().then(json => {
